...
\begin{definition}[Fraction of confounding] \hh{being nitpicky again: how do you get to q?} \al{This is where my ``simplifying'' choice to discuss a model with a single random effect I think made notation choices odd. I originally had $\ell$, which was the length of $\bm{b}$, but then I switched to $q$ since for a single random effect $\ell = q$ (I define q to be the length of $\bm{b}$ on page 8). When I made the switch I thought using $q$ would help reduce the notation flying around, but now that I start to think of the case with multiple random effects I am not so sure anymore.}
...
\al{Table~\ref{tab:results-int} and Figure~\ref{fig:results-slope}} display the estimated type I error rates using the AD normality test ($\alpha = 0.05$) on the rotated and varimax rotated random intercepts and random slopes, \al{respectively}, when $\sigma^2_\varepsilon = 4$ and $\sigma^2_{b_0} = \sigma^2_{b_1} = 1$. The other tests performed similarly, and full simulation results can be found in the supplementary material. Both figures show that the type I error rate is stabilized close to the nominal level with the appropriate choice of $s$. For the random intercept most choices of $s$ perform reasonably well, with the type I error rate closest to the nominal level for all error distributions between 30 and 40. For the random slope, $s$ must be chosen to be 30 for type I error to be near the nominal level; however, $s$ may need to be even smaller to achieve the nominal rate. 
...
\begin{figure}
	...
	\caption{\label{fig:power-slope}Linear smoother of the estimated power using the Anderson-Darling normality test ($\alpha = 0.05$) on the rotated random slopes (left) and varimax rotated random slopes (right) by $s$. The color denotes the distribution of the errors and the linetype denotes the distribution of the random slope. \al{The estimated power of the AD test for exponential and $t_3$ deviates is displayed in black.}}
\end{figure}
