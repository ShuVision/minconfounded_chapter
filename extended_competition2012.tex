...
\al{Section~\ref{sec:ex} illustrates} the inadequacy of existing methods based on the predicted random effects. We then introduce the concept of rotated residuals for the random effects and present a general method to obtain rotated residuals at all levels of the model \al{in Section~\ref{sec:methods}, which we evaluate with a simulation study in Section~\ref{sec:simulation}}. Finally, we demonstrate how this enables an appropriate graphical assessment  of distributional assumptions \al{in Section~\ref{sec:radon2}. We conclude with a discussion of how rotated residuals relate to other diagnostic procedures and suggest areas for future investigation.} 

\todo[inline]{Talk more about the dimensionality here}
