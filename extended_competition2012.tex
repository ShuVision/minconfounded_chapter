...
In order to correct residuals for the impact of confounding, we find weights $\bm{w_i}$ to apply to each of the residual contributions that minimize $\tr\left( \bm{B}\ginv\bm{A} \right)$. This can be re-expressed as the following minimization problem:
%\[
...
\end{equation}
where $\gamma_k$ and $\bm{w}_k$ are the $k$-th smallest eigenvalues and eigenvectors, respectively \citep{Fukunaga:1990}; thus, $\bm{W}^*$ consists of the eigenvectors associated with the $s$ \todo{revisit notation later} smallest eigenvalues. The computational formula for finding these eigenvalues is given below. The technical details can be found in the appendix.} \\
...
\[
\bm{W}^* = \argmin_{W \in \mathbb{R}^{\ell \times s} } 
\tr\left( \left(\bm{W\trans B W} \right)\inv \left(\bm{W\trans A W}\right) \right) = \bm{T_r \Lambda_r}^{-1/2} \bm{U}_s 
\]
where $\bm{T_r \Lambda_r}^{-1/2}$ is the full rank inverse square root of matrix $\bm{B}$ found through the spectral decomposition of $\bm{B}$, and $\bm{U}_s$ is the matrix comprised of 
the $s$ smallest eigenvectors of $ \bm{A^*} = (\bm{\Lambda_r}^{-1/2} \bm{T_r}\trans) \bm{A} (\bm{\Lambda_r}^{-1/2} \bm{T_r}\trans)\trans$. 
\end{theorem}

\al{
From theorem~\ref{thm1} we can easily calculate the rotated random effects, $\bm{W}^{*\prime} \widehat{\bm{b}}$, which are standardized, uncorrelated, and homoscedastic (see the appendix for a proof).
}

\todo[inline]{A paragraph is needed here discussing the choice of $s$.}
...
\todo[inline]{Now summarize the results. Below is my informal summary.}
...
\todo[inline]{The first part of this appendix is a mess right now. I am trying to rewrite the proof and I am not quite seeing the starting place... but I do know that it is true!}

\al{
We present the proof of theorem~\ref{thm1}. Following the developments presented in section~\ref{sec:rotate} we present this discussion for the random effects assuming that there is only a random intercept. Generalization to the situation with multiple random effects follows as previously discussed.
}
%To remain general we present our derivation in terms of the residual under investigation (the target residual), $\widehat{\bm{e}}$, which can refer to either $\widehat{\bm{\varepsilon}}$ or $\widehat{\bm{b}}$. 

In this section we will prove two claims:
\begin{enumerate}
\item $\bm{W}^* = \bm{T_r \Lambda_r}^{-1/2} \bm{U}_s$ minimizes
$\tr\left[ \left(\bm{W\trans B W} \right)\inv \left(\bm{W\trans A W}\right) \right]$
and consequently minimizes $\tr\left( \bm{B}\ginv\bm{A} \right)$.

\item The rotated residuals, $\bm{W}^{*\prime} \widehat{\bm{b}}$, are standardized, uncorrelated, and homoscedastic.

\end{enumerate}

\begin{proof}
%To prove this claim we appeal to two results, A.16 and A.17, from \cite{Fukunaga:1990} which give that 
%\[
% \frac{\partial}{\partial \bm{W}} 
% 	\tr\left( \left(\bm{W\trans B W} \right)\inv \left(\bm{W\trans A W}\right) \right)
% 	= 
% 	-2 \bm{BW} (\bm{W\trans B W})\inv (\bm{W\trans A W}) (\bm{W\trans B W})\inv + 2 \bm{W} (\bm{W\trans B W})\inv
%\]
%and that setting the above expression equal to zero we obtain
%\[
%(\bm{B}\inv \bm{A}) \bm{W} = \bm{W} (\bm{W\trans B W})\inv (\bm{W\trans A W})
%\]
%for symmetric $\bm{A}$ and $\bm{B}$.

 Let $\bm{A} = \var(\widehat{\bm{b}} | \bm{b})$, $\bm{B} = \var(\widehat{\bm{b}})$, $r = \text{rank}(\bm{B})$, and $\ell = $ the number of elements in $\widehat{\bm{b}}$. Note that by definition $\bm{A}$ and $\bm{B}$ are symmetric and nonnegative definite.  
