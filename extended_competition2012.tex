...
\begin{figure}[htb]
	...
	\caption{\label{fig:lineup} Lineup of normal Q-Q plots for the random slope term in model~\eqref{eq:radon}. The 19 null plots are obtained by simulation from the model. Can you identify the observed Q-Q plot?  \hh{XXX is this one of the plots we showed to people? If so, we could add how many people identified/did not identify the right plot }}
\end{figure}
...
the observed Q-Q plot (panel $3^2+7$) \hh{XXX there was some confusion by the reviewers about 12 + 4 - I think, the reviewer thought that 12 was the right answer} is virtually indistinguishable from the field of null plots. This suggests that the predicted random slopes  from the data do not deviate significantly from model~\eqref{eq:radon}.  
...
Notice that if $\bm{W}$ is the identity, then \eqref{eq:fc2} measures the fraction of confounding in the original vector of predicted random effects \hh{as proposed by  \cite{HildenMinton:1995wh}. XXX is that correct?}
...
\hh{Simultaneous diagonalization of $\bm{A}$ and $\bm{B}$ \citep{deLeeuw:1982to} can be used to solve this problem, given that the kernel of $\bm{B}$ is a subspace of the kernel of $\bm{A}$.
