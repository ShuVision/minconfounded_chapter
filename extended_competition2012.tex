...
\todo[inline]{Should we incorporate some discussion of the other simulations that I have run? If so where?}

To combat the issues presented above, we derive a reduced set of minimally confounded residuals that are standardized, uncorrelated, and homoscedastic. \al{Our development focuses on the random effects, but it is straightforward to generalize the results to residuals at other levels.} \cite{HildenMinton:1995wh} presents the derivation of a similar set of minimally confounded residuals for the error terms, but did not address the random effects. The derivation is relegated to the appendix, and presents a more general method that applies to residuals at all levels of the model.
...
\begin{equation}\label{eq:fc2}
\text{FC}(\widehat{\bm{e}}) = 
\frac{1}{\ell} \tr\left( \var(\widehat{\bm{e}})\ginv \  \var(\widehat{\bm{e}} | \bm{e} ) \right) = \frac{1}{\ell} \tr\left( \bm{B}\ginv\bm{A} \right)
...
In order to correct residuals for the impact of confounding, we to find weights $\bm{w_i}$ to apply to each of the residual contributions. This leads to the following minimization problem:
%\[
...
\tr\left( \left(\bm{W\trans B W} \right)\ginv \left(\bm{W\trans A W}\right) \right)
%\displaystyle{\sum_i} \frac{\bm{v_i}\trans \bm{W} \bm{A} \bm{W}\trans \bm{v_i}}
%		{\bm{v_i}\trans \bm{W} \bm{B} \bm{W}\trans \bm{v_i}}
\end{equation}
where $\ell$ is the length of vector $\bm{e}$ and $r = \text{rank}(\bm{B})$, \al{which can be solved using the generalized eigenvalue decomposition.} \\
...
\todo[inline]{Think about how to include discussions of the other rotations...}
