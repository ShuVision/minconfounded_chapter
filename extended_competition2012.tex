...
Inference for the fixed effects in linear mixed models is fairly robust against model mis-specification \citep{Butler:1992tx, Verbeke:1997tf}. This is different for random effects: the \al{empirical Bayes estimates of the random effects} \hh{XXX why are empirical Bayes estimates relevant? do you need to pull back so much? - how about: This is different for random effects, which are sensitive to distributional mis-specifications, e.g.~\citet{Verbeke:1996va} have shown XXX more details XXX for empirical Bayes estimates of random effects.} are sensitive to distributional mis-specifications \al{ \citep{Verbeke:1996va} }  and  therefore have to be checked carefully, especially when they are central to the inferential goals. %such as in the construction of a prediction interval for an unobserved group.
