...
%\hh{XXX why are empirical Bayes estimates relevant? do you need to pull back so much?} \al{XXX The empirical Bayes estimates are relevant because, under normality, the empirical Bayes estimates are the empirical BLUP, and the literature often discuses the EB estimates rather than the EBLUPS}  \hh{- how about: This is different for random effects, which are sensitive to distributional mis-specifications.} 
 \citet{Verbeke:1996va} have showed that the empirical Bayes estimates of the random effects can be severely distorted if the true distribution is not normal; thus, any distributional assumptions must be checked carefully when the random effects are central to the inferential goals.
...
For example, in small area estimation the areas are often modeled as random effects and their distribution is critical in the calculation of prediction intervals when the  within-group sample sizes are small \citep[see][for a discussion of this issue]{Jiang:2001dx}. 
...
The situation is not quite as dire as it might seem: it turns out that a lot of tests for normality are overly sensitive in assessing deviations of random effects from normality (see section~\ref{sec:sim-results} and simulation results in the supplement). By addressing the issue of over-sensitivity in evaluating normality, we are able to extend the applicability of normal model methods in a wide range of situations.
...
\begin{figure}[htb]
\centering
\includegraphics[width=0.4\textwidth]{figures/map.pdf}
...
\begin{figure}[hbt]
\centering
\includegraphics[width=0.6\linewidth]{figures/radon-twocounties.pdf}
...
\begin{figure}[!h]
	\centering
	  \subfloat[Predicted error terms]{
		\includegraphics[width=0.3\linewidth]{raw-lev1-qq.pdf}
	   }
	  \subfloat[Random intercepts]{
		\includegraphics[width=0.3\linewidth]{raw-intercept-qq.pdf}
		}
...
\begin{figure}[htb]
	\centering
	\includegraphics[width=0.75\textwidth]{qq-lineup-std.pdf}%test.jpeg}%lineup-rslope.pdf}
...
the observed Q-Q plot (panel \#$(3^2+7)$) is virtually indistinguishable from the field of null plots. This suggests that the predicted random slopes  from the data do not deviate significantly from model~\eqref{eq:radon}.  Note that an alternative display of the residuals did produce similar results (see section 3 of the supplement).
...
The use of standardized residuals can correct the latter issue, but does not address the fact that the residuals are interrelated. While problems may be expected at both levels of the model based on \eqref{eq:resid1} and \eqref{eq:resid2}, we have found that the interpretation of Q-Q plots of the standardized predicted error terms, $\bm{z}_{\varepsilon} =  \text{diag} \left(\bm{RPR} \right)^{-1/2} \widehat{\bm{\varepsilon}}$,
is unaffected by this interrelationship in the situations that we have considered (see section 4 of the supplement for simulation results). %XXX Reviewer 2 thinks that this may not hold for small cluster sizes, so I am trying to soften the language just a bit. We didn't exhaust all possibilities in our simulation, but the results we have support this statement
...
Equivalent to that, we have to show that the difference $\bm{B} - \bm{A}$ is nonnegative definite (n.n.d.):
$\bm{A} = \bm{DZ}^\prime \bm{PRPZD}$
and
$\bm{B} = \bm{DZ}^\prime \bm{PRPZD} + \bm{DZ}^\prime\bm{PZDZ}^\prime\bm{PZD}$,
so $\bm{B}-\bm{A} =   \bm{DZ}^\prime\bm{PZDZ}^\prime\bm{PZD}$ is n.n.d.
...
Normal, heavy-tailed, and skewed distributions were used to generate the simulated errors and random effects. We used a rescaled $t$ distribution with 3 degrees of freedom to generate heavy tailed residuals, and a centered and rescaled exponential distribution with a rate parameter of 1 to generate skewed residuals. For simplicity we required the distributions of the random slope and intercept to the be same and assumed independence between the random effects; thus, nine distributional settings were considered in the simulation study.
%
%\begin{table}[htdp]
%\centering
...
To investigate the effect that pooling has on the rotated random effects we considered  three variance structures to represent different degrees of confounding for the random effects: $\sigma^2_\varepsilon = 4$ and  $\sigma^2_{b_0} = \sigma^2_{b_1} = 1$ (high); $\sigma^2_\varepsilon = 1$ and  $\sigma^2_{b_0} = \sigma^2_{b_1} = 1$ (moderate); $\sigma^2_\varepsilon = 1$ and  $\sigma^2_{b_0} = \sigma^2_{b_1} = 4$ (low).
%
%\begin{tabular}{ll}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.75\textwidth]{fc_by_s.pdf}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.75\textwidth]{ad_slope_results.pdf}
...
\begin{figure}
	\centering
	\includegraphics[width=0.75\textwidth]{ad_slope_power.pdf}
...
\begin{figure}
\centering
	\includegraphics[width=0.65\textwidth]{radon_elbows.pdf}
...
\begin{figure}[htb]
	\centering
	 \subfloat[Random intercept]{
		\includegraphics[width=0.3\linewidth]{rotatedQQ-intercept.pdf}
		}
	  \subfloat[Random slope]{
		\includegraphics[width=0.3\linewidth]{rotatedQQ-slope.pdf}
		}	
...
\begin{description}
\item[Technical details and simulation results:] The supplementary materials include additional technical details and the results of the full simulation study. Additionally, the results of a simulation supporting the small simulation study discussed in Section~\ref{sec:ex} are presented and further show the need for alternative procedures to assess the distribution of the random effects.
...
\item[\texttt{R} package \texttt{HLMdiag}:] We have included the function to calculate the rotated random effects in the \texttt{R} package \texttt{HLMdiag}, which can be found on the Comprehensive R Archive Network (\url{cran.r-project.org/}) and github (\url{github.com/aloy/HLMdiag}).
\end{description}
