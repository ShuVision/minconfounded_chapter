...
\subfloat[Random intercept]{
	...
	& \multicolumn{3}{c}{Test} \\ \cline{2-4}
	  	&     AD & CVM  & KS \\ \hline
	Normal       	   &	 0.655 & 0.615 & 0.494  \\ 
	Heavy tailed 	   & 0.890 & 0.878 &  0.785  \\ 
	Skewed       	   & 0.841 & 0.830 &  0.715  \\  
	\hline
	\end{tabular}
}
\qquad
\subfloat[Random slope]{
	...
	& \multicolumn{3}{c}{Test} \\ \cline{2-4}
	  	&     AD & CVM  & KS \\ \hline
	Normal       	  & 0.874 &  0.869 & 0.815  \\ 
	Heavy tailed 	  &	0.965 &	 0.967 &	 0.927  \\ 
	Skewed       	  &	0.953 &	 0.956 &	 0.909  \\  
	\hline
	\end{tabular}
}
...
Let $\bm{b}$ denote the vector of random effects and $\widehat{\bm{b}}$ its predictions as defined in \eqref{eq:resid2}. For a full rank matrix $\bm{W} \in \mathbb{R}^{q \times s}$, where $ s \le q$, the fraction of confounding in the $s$-dimensional space spanned by $\bm{W}$ is defined as 
\begin{equation}\label{eq:fc2}
\text{FC}(W; \widehat{\bm{b}}) = \frac{1}{q} \tr\left( \left(\bm{W\trans B W} \right)\ginv \left(\bm{W\trans A W}\right) \right),
%\dfrac{1}{\ell} \displaystyle{\sum_i} \frac{\bm{v_i}\trans \bm{A} \bm{v_i}}
%		{\bm{v_i}\trans \bm{B} \bm{v_i}}.
\end{equation}
where $\bm{B}$ is the covariance structure of $\bm b$, $\bm{B} = \var(\widehat{\bm{b}})$, and $A$ is the conditional covariance structure of  $\widehat{\bm{b}}$ given $\bm{b}$,  i.e. $\bm{A} = \var(\widehat{\bm{b}} | \bm{b} )$.
\end{definition}
...
\al{Selection of the dimension of the subspace spanned by the rotated residuals is central to our proposed method. Ideally, we would select the dimension such that the fraction of confounding is reduced to zero; however, this is not realistic in practice. Alternatively, we propose choosing the dimension that provides a substantial reduction in the fraction of confounding. Since our ultimate objective is distributional assessment, we must balance this reduction in the fraction of confounding with the loss in power of a test of the empirical distribution function (e.g., the Anderson-Darling test for normality) associated with dimension reduction. To guide this choice we suggest plotting the reduction in the fraction of confounding against the reduction in dimension, which is similar to the scree plot used to select the number of principal components. To illustrate the use of this plot we simulated two simple random intercept models with a group-level predictor: the first model had 40 groups of 30 observations and 10 groups of 5 observations; the second model also had 50 groups, with group sizes determined as random draws from either a Poisson(30) distribution (40 groups) or a Poisson(5) distribution (10 groups). Figure~\ref{fig:elbow} shows two examples of such plots constructed for the simulated models. Both figures have an ``elbow'' in the plot corresponding to a reduction in the dimension of the subspace of 11; thus we would choose $s = 39$.}
%
\begin{figure}[htb]
	\centering
	 \subfloat[Model 1]{
		\includegraphics[width=0.49\linewidth]{elbow1.pdf}
		}
	  \subfloat[Model 2]{
		\includegraphics[width=0.49\linewidth]{elbow2.pdf}
		}	
	\caption{\label{fig:elbow} Plots of the fraction of confounding for each reduction in the dimension of subspace spanned by the rotated random intercepts from two simulation models. Model 1 has 50 groups: 40 groups of size 30, and 10 groups of size 5. Model 2 has 50 groups with group sizes determined as random draws from either a Poisson(30) distribution (40 groups) or a Poisson(5) distribution (10 groups). Based on these plots we would choose $s = 39$, corresponding to a dimension reduction of 11.}
\end{figure}
...
Distributions of & & \multicolumn{3}{c}{Random effects, $F_2$} \\ 
...
\todo[inline]{Change the figure/table references once we set the final tables.}
...
%\begin{sidewaystable}
%\centering
%\caption{Proportions of AD tests rejecting the null hypothesis of normality at the .05 significance level for the rotated (Rot) and varimax rotated (Vmx) random intercepts by $s$. The gray shading indicates the situations where the random intercept is normal (i.e., type I error). \al{This could be made into 2 tables: 1 for Rot, 1 for Vmx}}
%\begin{footnotesize}
...
%Normal	&	Normal			&	0.04	&	0.05	&	0.04	&	0.05	&	0.05	&	0.05	&	0.04	&	0.05	&	0.04	&	0.05	&	0.05	&	0.05	&	0.06	&	0.06	\\
%	    &	Heavy tailed		&	0.07	&	0.09	&	0.08	&	0.09	&	0.07	&	0.08	&	0.06	&	0.07	&	0.05	&	0.06	&	0.04	&	0.05	&	0.05	&	0.05	\\
%	    &	Skewed			&	0.05	&	0.05	&	0.05	&	0.05	&	0.06	&	0.04	&	0.06	&	0.06	&	0.06	&	0.06	&	0.06	&	0.05	&	0.06	&	0.06	\\
...
\begin{table}
\centering
\caption{Proportions of AD tests rejecting the null hypothesis of normality at the .05 significance level for the rotated (a) and varimax rotated (b) random intercepts by $s$. The gray shading indicates the situations where the random intercept is normal (i.e., type I error). \al{I will change this to \% with 1 decimal place, but I wanted to reformat and have something here to before I reran the formatting code.}}
...
\rowcolor{gray!20}Normal	&	Normal			& 0.04	& 	0.04	& 0.05	& 0.04	&	0.04	&	0.05	&	0.06 \\
\rowcolor{gray!20}	    &	Heavy tailed		& 0.07	&	0.08	& 0.07	& 0.06	&	0.05	&	0.04	&	0.05 \\	
\rowcolor{gray!20}	    &	Skewed			& 0.05	&	0.05	& 0.06	& 0.06	&	0.06	&	0.06	&	0.06 \\	
&&&&&&&&\\
Heavy tailed	&	Normal		& 0.14	&	0.14	& 0.13	& 0.13	&	0.13	&	0.13	&	0.12 \\	
	        & Heavy tailed	& 0.19	&	0.19	& 0.17	& 0.16	&	0.16	&	0.15	&	0.14 \\	
			& Skewed			& 0.15	&	0.15	& 0.14	& 0.14	&	0.13	&	0.13	&	0.12 \\
&&&&&&&&\\
Skewed	&	Normal			& 0.10	&	0.09	& 0.10	& 0.10	&	0.10	&	0.11	&	0.10 \\	
		&	Heavy tailed		& 0.13	&	0.12	& 0.12	& 0.11	&	0.10	&	0.11	&	0.11 \\	
		&	Skewed			& 0.13	&	0.13	& 0.12	& 0.12	&	0.11	&	0.11	&	0.11 \\
\hline
	\end{tabular}
}
...
\rowcolor{gray!20}	    &	Heavy tailed		& 	0.09	&	0.09	&	0.08	&	0.07	&	0.06	&	0.05	&	0.05	\\
\rowcolor{gray!20}	    &	Skewed			& 	0.05	&	0.05	&	0.04	&	0.06	&	0.06	&	0.05	&	0.06	\\
&&&&&&&&\\
Heavy tailed	&	Normal		& 	0.22	&	0.22	&	0.23	&	0.23	&	0.23	&	0.22	&	0.22	\\
	        & Heavy tailed	& 	0.34	&	0.33	&	0.32	&	0.32	&	0.30	&	0.27	&	0.27	\\
			& Skewed			& 	0.28	&	0.27	&	0.26	&	0.27	&	0.24	&	0.23	&	0.22	\\
&&&&&&&&\\
Skewed	&	Normal			& 	0.20	&	0.21	&	0.21	&	0.22	&	0.23	&	0.21	&	0.22	\\
		&	Heavy tailed		& 	0.30	&	0.28	&	0.27	&	0.25	&	0.26	&	0.25	&	0.24	\\
		&	Skewed			& 	0.22	&	0.23	&	0.22	&	0.23	&	0.21	&	0.23	&	0.21	\\
\hline
	\end{tabular}
}
\end{table}
...
%
\todo[inline]{We could show this in a plot if room isn't an issue.}
\begin{table}[ht]
\centering
